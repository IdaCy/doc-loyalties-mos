...
def maybe_load_policy_adapter(model: Any, config: dict[str, Any]) -> Any:
    init_adapter = get_key(config, "warmstart.init_adapter_path") or get_key(config, "sft.init_adapter_path")
    if not init_adapter:
        return apply_lora_if_needed(model, config)
    try:
        from peft import PeftModel
    except ModuleNotFoundError as exc:
        raise SystemExit("missing peft dependency for PPO warm-start init adapter") from exc
...
def write_summary(run_dir: Path, config: dict[str, Any], train_rows: int, eval_rows: int | None) -> None:
    payload = {
        ...
        "init_adapter_path": get_key(config, "warmstart.init_adapter_path") or get_key(config, "sft.init_adapter_path"),
...
def run_sft_training(config: dict[str, Any], run_dir: Path) -> Path:
    ...
    model = maybe_load_policy_adapter(model, config)
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--config", default="configs/ppo_warmstart_sft_nano.yaml")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if get_key(config, "run.method") != "ppo_warmstart_sft":
        raise ValueError(f"expected run.method=ppo_warmstart_sft, got {get_key(config, 'run.method')}")
    if args.dry_run:
