...
from .train_ppo_warmstart_sft import apply_lora_if_needed, build_training_args, chat_text, filtered_kwargs, model_kwargs
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--config", default="configs/ppo_warmstart_dpo_nano.yaml")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if get_key(config, "run.method") != "ppo_warmstart_dpo":
        raise ValueError(f"expected run.method=ppo_warmstart_dpo, got {get_key(config, 'run.method')}")
    if args.dry_run:
